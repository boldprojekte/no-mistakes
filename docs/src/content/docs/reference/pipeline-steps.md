...
- Runs a targeted Simplification pass over introduced branches, acceptance or matching paths, fallbacks, aliases, modes, flags, options, duplicate definitions, and parallel rules. These are candidates, not automatic findings: absence from the literal intent is insufficient. A finding requires source evidence that the change materially adds concepts, behavior, maintenance burden, or failure surface unnecessary for the accepted intent and that removal or narrowing preserves correctness and that intent. Architecture preferences, code smells, maintainability concerns, and hypothetical evolvability are not findings on their own. Non-functional simplification stays inside the changed area and does not authorize unrelated refactoring, broad deduplication, new abstractions, future-proofing, or cleanup

...
[`review.strategy: bounded`](/no-mistakes/reference/repo-config/#reviewstrategy) ignores `auto_fix.review`: its implementation worker must explicitly disposition every finding, and its confirmed fixes always use at most one consolidated correction.
